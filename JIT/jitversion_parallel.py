...
# Additional method to use multiprocess to improve JIT implementation 
def run_multi(all_u0, mask, MAX_ITER, ABS_TOL):
    args = [(u0, mask, MAX_ITER, ABS_TOL) for u0, mask in zip(all_u0, mask)] # batches of params for tasks

    # 16 process because XeonGold6226R in HPC has 16 cores 
    with mp.Pool(processes=16) as pool:
        results = pool.starmap(jacobi_numba, args) # starmap to use mutliple params
    return results
...
if __name__ == '__main__':
    ...
    u_solutions = run_multi(all_u0.copy(), all_interior_mask, MAX_ITER, ABS_TOL) 
    for bid, u0, mask in zip(building_ids, u_solutions, all_interior_mask):
        stats = summary_stats(u0, mask)
